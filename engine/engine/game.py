...
class Game(Deck):
    ...
    def play(self, play0: List[str | None], play1: List[str | None]) -> None:
        if not len(play0) == self.play_len or not len(play1) == self.play_len: raise PlayIncorrect("Nombre de cartes jouées incorrect")
        plays = [play0, play1]
        for player in range(2):
            for k in range(self.play_len):
                if plays[player][k] is None: continue
                self.score[self.round, self.turn, player] += self.decks[player].cards[plays[player][k]].power
